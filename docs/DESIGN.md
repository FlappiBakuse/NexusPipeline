...
- **判定交给用户**：运行结果由「完成判定」驱动——优先判断脚本（用户自写 JS/Python，专用插件判定由插件固化脚本驱动），其次成功/失败关键字与通用完成标志（同一行匹配机制）；未配置任何判定时按「进程自行退出」判成功。判定输入为**本次尝试日志段**，跨尝试互不污染。
...
| 完成判定（SessionJudge） | 判断脚本/关键字两模式的判定状态机（完成标志 ≡ 只有成功组的关键字模式），每尝试独立实例 |
...
判断脚本（启用即优先，忽略关键字）→ 成功/失败关键字（行内逗号 AND、换行 OR）→ 通用完成标志 → 无任何配置按「进程自行退出」判定成功。

> **v0.6.0+ 专用插件判定**：专项脚本实例的判断脚本由插件固化（`ApplyProfile` 保存时覆盖 `JudgeScriptEnabled/Language/JudgeScript`，用户不可编辑），判定走脚本模式，不再依赖插件提供关键字/标志；`ScriptProfile.SuccessMarkers` 已不再由随附插件提供（v0.6.2 起），专项实例固化 `successMarkers` 为空。通用完成标志（`successMarkers`）仍供通用脚本与历史实例使用。
...
- **完成标志 ≡ 只有成功组的关键字模式**（v0.6.2 合并实现）：每个标志为一个单元素成功组，行内出现即命中，与关键字行匹配同机制（`SessionJudge` 无独立标志分支）；成功命中同样等待退出 60 秒。来源为通用脚本手填的 `successMarkers`（API 字段）与历史配置的遗留实例。
- 专用插件判定：v0.6.0+ 判断脚本由插件固化（用户不可编辑，前端专项弹窗不渲染自定义完成标志区；后端 `ApplyProfile` 兜底清空关键字字段），随附四个插件均不再提供完成标志（v0.6.2 起 `ScriptProfile.SuccessMarkers` 不再由随附插件返回）。
