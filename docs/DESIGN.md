...
| 全局用户（NexusUser） | 具有稳定 `UserId` 的账号实体：可改用户名、全局优先级、头像和自动签到占位；可绑定多个脚本实例 |
...
- 每次「脚本实例 × 全局用户绑定」运行结束保存（v0.5.3 精简：纯状态 + 按尝试分批日志）：
...
### 8.1 语义保留项审计（v0.10.0，台账见 KNOWN_ISSUES.md）
...
| 定时触发为秒级 tick，跨整点/休眠错过不补跑 | 不补跑：该分钟内处于运行状态即可触发，错过即错过 | 文档 §8 第 4 条 + L1 测试 `Scheduler_DoesNotBackfillMissedOccurrence` |
| `IsExeRunning` 按进程名检测同名进程可能误报 | 保守优先：宁可误报防重复启动 | 文档 §8 第 7 条 |
| 通知单通道失败仅告警不阻断 | 通道隔离：一通道异常不影响其余通道与运行流程 | 文档 §7.1 + 既有测试 |
