...
3. 本地验证：`build.cmd` → `node uitest\test.mjs`（全量 e2e，353 项断言）全绿，或先跑核心回归集 `node uitest\test.mjs --ci`（330 项断言）；
...
3. 打 tag：`git tag vX.Y.Z` → `git push origin vX.Y.Z`（发布操作先经用户同意）；
4. 编写 release notes 到临时文件（`gh pr create --body` 引号坑同理，用 `--notes-file`）；
5. `gh release create vX.Y.Z --prerelease --title vX.Y.Z --notes-file <file>`；
6. 上传资产：`gh release upload vX.Y.Z NexusPipeline-vX.Y.Z-win-x64.zip NexusPipeline-vX.Y.Z-win-x64.zip.sha256`；
7. 校验：`Get-FileHash` 与 `.sha256` 内容一致；下载 zip 重新计算复核。
