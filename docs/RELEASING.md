...
1. 确认本地构建与测试全绿：
   - `build.cmd`（提权版）；
    - 单元测试 `dotnet test src\NexusPipeline.Tests\NexusPipeline.Tests.csproj --nologo`（183 断言）；
   - **真实计时档**（不设 `NEXUS_TIME_SCALE`）全量回归：e2e 77 + judge-scenarios 150 + chaos-queue 166。
