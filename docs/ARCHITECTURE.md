...
├── src/NexusPipeline.Plugin.Abstractions/  独立 public Plugin API v1.2（无宿主业务引用）
...
- **Plugins 通过数据化 manifest 或独立 Plugin API v1.2 交互**；`NexusPipeline.Plugin.Abstractions` 不引用宿主业务模型，managed-code 插件由 collectible `AssemblyLoadContext` 隔离加载；跨模块的宿主内部 capability/profile 契约位于 `Extensibility/`，数据化专项插件（`DataSpecializedPlugin`）仍为纯数据驱动。
...
| `NotificationDispatcher` | src/Services/Notification/NotificationDispatcher.cs | 宿主内置 Webhook/SMTP 通知领域服务；脚本、队列和 Plugin API v1.2 DTO 均从此入口发送 |
...
- 主程序程序集仍只向自身暴露 `Program`（入口）与领域模型；外部代码插件只引用独立的 `NexusPipeline.Plugin.Abstractions` public API v1.2。宿主内部的 `IPluginCapability`/`ScriptProfile` 不属于外部插件契约，Plugin API 不暴露宿主 DI 或领域模型。
...
| 类别 | 形态 | 职责 | 启用语义 |
|---|---|---|---|
| managed-code 插件 | 独立项目 + `NexusPipeline.Plugin.Abstractions` API v1.2 + manifest | 通过通用用户数据、声明式设置、用户列表徽章、用户运行事件、HTTP 和通知端口实现插件能力 | 默认禁用；启用后重启加载，API 不兼容或初始化失败会进入对应运行态 |
...
- 数据插件 capability 通过 key 登记；managed-code 插件只通过 API v1.2 服务端口工作，宿主不把后台任务 capability 当作专项脚本选择器。
...
- Plugin API v1.2 只提供显式 `IPluginHostContext` / `IPluginHostContextV1_1` / `IPluginHostContextV1_2` 服务端口；插件全局配置、插件级密钥和按用户配置/密钥分层存储于 `config/plugins/`，managed-code 插件停止时后台任务、用户设置贡献、用户列表徽章和事件订阅统一取消。
...
- managed-code 插件实现独立 API 项目的 `INexusPlugin` 生命周期，并通过 `IPluginHostContextV1_2` 使用宿主提供的通用用户数据、声明式用户全局管理、用户列表徽章、用户运行事件、HTTP、日志、通知和任务端口。
