...
/// </summary>
public sealed class ConfigInputsPerUserTestsFixture
{
internal static (string PluginDir, string ScriptRoot) MakeOneDragonLikePlugin(bool requiredConfig = false)
    {
...
internal sealed class PluginBackedCapabilities(DataSpecializedPlugin plugin) : IPluginCapabilityResolver
    {
...
internal sealed class AvailablePluginAvailability : IPluginAvailability
    {
...
internal sealed class SingleScriptRepository(ScriptInstance script) : IScriptRepository
    {
...
internal sealed class SingleQueueRepository(DispatchQueue queue) : IQueueRepository
    {
...
internal sealed class TestUsers(params NexusUser[] users) : CurrentModelUserRepository(users);
}
