...
public sealed class ScriptPersistenceTests
{
    ...
    public void ConfigPathChange_RebuildsStoreFromCurrentLocation()
    {
        ...
        try
        {
            ...
            bool prepared = ConfigExchangeService.PrepareForRun(
                scriptId,
                userId,
                oldConfig,
                out string? prepareError);
            Assert.True(prepared, prepareError);
            Assert.Null(ConfigExchangeService.RestoreAfterRun(scriptId, userId, oldConfig));
            Assert.True(File.Exists(oldConfig));
            Assert.NotNull(ConfigStoreMetadata.Load(scriptId, userId));

            bool reused = ConfigExchangeService.PrepareForRun(
                scriptId,
                userId,
                newConfig,
            ...
            Assert.Equal("{\"state\":\"old\"}", File.ReadAllText(Path.Combine(
                ConfigPaths.StoreDir(scriptId, userId), "old.json")));
            File.WriteAllText(newConfig, "{\"state\":\"new\"}");
            bool rebound = ConfigExchangeService.PrepareForRun(
                scriptId,
                userId,
                newConfig,
            ...
            Assert.Null(ConfigExchangeService.RestoreAfterRun(scriptId, userId, newConfig));
            Assert.Equal("{\"state\":\"new\"}", File.ReadAllText(Path.Combine(
                ConfigPaths.StoreDir(scriptId, userId), "new.json")));
    ...
    public void CorruptSessionMarks_PreserveConfigAndCacheWithoutGuessing()
    {
        ...
        try
        {
            ...
            string cache = ConfigPaths.CacheDir(scriptId, userId);
