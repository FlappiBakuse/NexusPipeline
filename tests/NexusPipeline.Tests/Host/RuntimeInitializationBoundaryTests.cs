...
public sealed class RuntimeInitializationBoundaryTests
{
    ...
    public void HostedEntityInitializationOccursAfterSingleInstanceAcquisition()
    {
        ...
        string source = File.ReadAllText(Path.Combine(root, "src", "Host", "Lifecycle", "StartupPipeline.cs"));
    ...
    {
        string root = FindProjectRoot();
        string source = File.ReadAllText(Path.Combine(root, "src", "Host", "Lifecycle", "Bootstrap.cs"));
    ...
    {
        string root = FindProjectRoot();
        string source = File.ReadAllText(Path.Combine(root, "src", "Modules", "Settings", "Persistence", "AppSettingsStore.cs"));
