...
public sealed class PluginNameMigrationTests
{
    ...
    public void Apply_MigratesPersistedScriptBindingAcrossReload()
    {
        ...
        ScriptDefinitionStore.SaveScripts(new List<ScriptInstance> { script });
        ...
        Assert.Equal(PluginNameMigration.MaaStellaSora, Assert.Single(ScriptDefinitionStore.LoadScripts()).PluginType);
        context.ReloadData();
        Assert.Equal(PluginNameMigration.MaaStellaSora, Assert.Single(context.EntityState.SnapshotScripts()).PluginType);
    }
    ...
    private static void ApplyMigration(HostCompositionRoot context)
    {
        PluginNameMigration.Apply(
            context.Settings,
            context.EntityState.SnapshotScripts(),
            context.ReplaceSettings,
            scripts => context.EntityState.Mutate(state =>
            {
                state.Scripts.Clear();
