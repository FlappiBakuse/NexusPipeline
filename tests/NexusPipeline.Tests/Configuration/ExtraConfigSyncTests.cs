...
public class ExtraConfigSyncTests
{
    ...
    public void PrepareAdoptsSiteIntoEmptyStoreThenCoversSiteFromStore()
    {
        ...
        try
        {
            ...
            string storeExtra = ConfigPaths.StoreExtraDir(scriptId, userKey, site);
            // 首次 adopt：快照等于现场
            ...
            string originalExtra = ConfigPaths.OriginalExtraDir(scriptId, userKey, site);
    ...
    public void RestorePutsSiteBackupBackAndClearsOriginal()
    {
        ...
        try
        {
            ...
            Assert.False(Directory.Exists(ConfigPaths.OriginalExtraDir(scriptId, userKey, site)));
        }
        finally
        {
            Cleanup(scriptId);
    ...
    public void PrepareFailure_RollsBackEarlierExtraPathsAndBlocksPreparation()
    {
        ...
        try
        {
            ...
            Assert.False(Directory.Exists(ConfigPaths.OriginalExtraDir(scriptId, userKey, first)));
    ...
    public void Recovery_UncommittedExtraStoreSwapRestoresOldSnapshot()
    {
        ...
        string store = ConfigPaths.StoreExtraDir(scriptId, userKey, site);
        string transactionDir = ConfigPaths.ExtraStoreTransactionDir(scriptId, userKey, site);
        try
        {
            Directory.CreateDirectory(Path.GetDirectoryName(site)!);
            ...
            string backup = ConfigPaths.ExtraStoreTransactionBackupDir(scriptId, userKey, site);
            Directory.CreateDirectory(transactionDir);
            Directory.Move(store, backup);
            Directory.CreateDirectory(store);
            File.WriteAllText(Path.Combine(store, "config.json"), "未提交新快照");
            JsonUtil.WriteAtomic(
                ConfigPaths.ExtraStoreTransactionManifestPath(scriptId, userKey, site),
    ...
    public void SyncWritesSiteDifferencesIntoStoreIncludingDeletes()
    {
        ...
        try
        {
            ...
            string storeExtra = ConfigPaths.StoreExtraDir(scriptId, userKey, siteDir);
    ...
    {
        string a = ConfigPaths.ExtraKey("D:/games/DATA/CONFIGS/software_config.json");
        string b = ConfigPaths.ExtraKey("D:/games/DATA/CONFIGS/software_config.json");
        string c = ConfigPaths.ExtraKey("D:/games/User/config.json");
        Assert.Equal(a, b);
        Assert.NotEqual(a, c);
