...
public class ConfigEditModesTests
{
    ...
    public void PrepareForRun_EmptyStore_CopiesConfigAsInitialSnapshot()
    {
        ...
        try
        {
            Assert.True(ConfigExchangeService.PrepareForRun(scriptId, userName, configPath, out string? error), error);
            Assert.Null(error);

            // 初始快照已建立且内容为现场配置
            Assert.True(ConfigSnapshotService.HasSnapshot(scriptId, userName));
            Assert.Equal("user-current", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
            // 交换语义不变：config 位置是快照副本，original 缓存是原配置
            Assert.Equal("user-current", ReadFile(configPath));
            Assert.Equal("user-current", ReadFile(Path.Combine(ConfigPaths.CacheDir(scriptId, userName), "config.json")));
        }
        finally
        {
            ConfigExchangeService.RestoreAfterRun(scriptId, userName, configPath);
    ...
    public void PrepareForRun_MissingConfig_KeepsEmptyStore()
    {
        ...
        try
        {
            ...
            Assert.False(ConfigSnapshotService.HasSnapshot(scriptId, userName));
            Assert.False(Directory.Exists(ConfigPaths.StoreDir(scriptId, userName)));
        }
        finally
        {
            ConfigExchangeService.RestoreAfterRun(scriptId, userName, configPath);
    ...
    public void FreshStart_WithExistingConfig_MovesConfigToCache()
    {
        ...
        try
        {
            Assert.Null(ConfigEditSessionService.PrepareForEditFresh(scriptId, userName, configPath));

            // 原配置移入缓存区，config 位置为空（脚本将在此生成新配置）
            Assert.False(File.Exists(configPath));
            Assert.Equal("original", ReadFile(Path.Combine(ConfigPaths.CacheDir(scriptId, userName), "config.json")));
            ...
            Assert.Null(ConfigEditSessionService.CommitEdit(scriptId, userName, configPath));
            Assert.Equal("generated", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
    ...
    public void FreshCancel_RemovesGenerated_AndRestoresOriginal()
    {
        ...
        try
        {
            ...
            Assert.Null(ConfigEditSessionService.CancelEdit(scriptId, userName, configPath));
    ...
    public void FreshWithExtra_FirstEditCommitCreatesExtraSnapshot_AndRestoresOriginalSite()
    {
        ...
        try
        {
            Assert.Null(ConfigEditSessionService.PrepareForEditFresh(
                scriptId,
                userName,
                configPath,
                extraConfigPaths: [extraPath]));

            string extraStore = ConfigPaths.StoreExtraDir(scriptId, userName, extraPath);
            ...
            Assert.Equal("main-generated", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
    ...
    public void FreshWithExtra_CancelDoesNotCreateExtraSnapshot()
    {
        ...
        try
        {
            ...
            Assert.False(Directory.Exists(ConfigPaths.StoreExtraDir(scriptId, userName, extraPath)));
            Assert.Equal("main-original", ReadFile(configPath));
            Assert.Equal("{\"value\":\"extra-original\"}", ReadFile(extraPath));
            Assert.Null(ConfigSessionMark.TryRead(scriptId, userName));
        }
    ...
    public void ReuseStart_NoFileAction()
    {
        ...
        try
        {
            Assert.Null(ConfigEditSessionService.PrepareForEditReuse(scriptId, userName, configPath));

            // 无任何文件动作：config 原位原内容，无缓存区、无快照
            Assert.Equal("user-content", ReadFile(configPath));
            Assert.False(Directory.Exists(ConfigPaths.CacheDir(scriptId, userName)));
            Assert.False(ConfigSnapshotService.HasSnapshot(scriptId, userName));
            ConfigSessionMark? mark = ConfigSessionMark.TryRead(scriptId, userName);
            Assert.NotNull(mark);
        ...
        {
            ConfigEditSessionService.CancelEdit(scriptId, userName, configPath);
            Cleanup(scriptId, tempRoot);
        }
    }
    ...
    public void ReuseDone_CopiesConfigToStore_AndKeepsConfigInPlace()
    {
        ...
        try
        {
            ...
            Assert.Equal("edited", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
    ...
    public void ReuseCancel_ClearsPendingInputWithoutCreatingSnapshot()
    {
        ...
        try
        {
            Assert.Null(ConfigEditSessionService.PrepareForEditReuse(
                scriptId,
                userName,
                configPath,
                options: options));
    ...
    public void NormalStart_MaterializesSnapshot_AndCommitWritesDiffBack()
    {
        ...
        try
        {
            ...
            File.WriteAllText(configPath, "user-current");
            Assert.True(ConfigExchangeService.PrepareForRun(scriptId, userName, configPath, out string? runError), runError);
            Assert.True(ConfigSnapshotService.HasSnapshot(scriptId, userName));

            Assert.Null(ConfigEditSessionService.PrepareForEdit(scriptId, userName, configPath));
            File.WriteAllText(configPath, "edited-in-target");

            Assert.Null(ConfigEditSessionService.CommitEdit(scriptId, userName, configPath));
            Assert.Equal("edited-in-target", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
            Assert.Null(ConfigSessionMark.TryRead(scriptId, userName));
        }
        finally
        {
    ...
    public void NormalCommit_WhenConfigRenamedAway_FailsWithMissingLocation()
    {
        ...
        try
        {
            ...
            string? commitError = ConfigEditSessionService.CommitEdit(scriptId, userName, configPath);
    ...
    public void NormalMultiCandidateIsolation_MovesSiblings_AndRestoresThemOnCommit()
    {
        ...
        {
            Assert.True(ConfigExchangeService.PrepareForRun(scriptId, userName, configPath, out string? runError), runError);
            Assert.Null(ConfigExchangeService.RestoreAfterRun(scriptId, userName, configPath));

            var options = new ConfigEditPreparationOptions(true, [siblingA, configPath, siblingC]);
            Assert.Null(ConfigEditSessionService.PrepareForEdit(
                scriptId,
                userName,
                configPath,
            ...
            Assert.True(Directory.Exists(ConfigPaths.EditIsolationDir(scriptId, userName)));

            File.WriteAllText(configPath, "target-edited");
            Assert.Null(ConfigEditSessionService.CommitEdit(scriptId, userName, configPath));

            Assert.Equal("target-edited", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json")));
            Assert.Equal("target-original", ReadFile(configPath));
            Assert.Equal("a-original", ReadFile(siblingA));
            Assert.Equal("c-original", ReadFile(siblingC));
            Assert.False(Directory.Exists(ConfigPaths.EditIsolationDir(scriptId, userName)));
            Assert.Null(ConfigSessionMark.TryRead(scriptId, userName));
        }
        finally
        {
    ...
    public void ReuseMultiDirectoryIsolation_CopiesSelectedWorking_AndRestoresAllDirectories()
    {
        ...
        try
        {
            ...
            Assert.Null(ConfigEditSessionService.CommitEdit(scriptId, userName, selected));

            Assert.Equal("two-edited", ReadFile(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "value.txt")));
    ...
    public void CurrentFreshMissingMark_RecoveryClearsGeneratedConfig()
    {
        ...
        try
        {
            ...
            ConfigRecoveryService.RecoverInterrupted([user]);
    ...
    public void HasSnapshot_TrueOnlyWhenStoreHasContent()
    {
        ...
        {
            Assert.False(ConfigSnapshotService.HasSnapshot(scriptId, userName));
            Directory.CreateDirectory(ConfigPaths.StoreDir(scriptId, userName));
            Assert.False(ConfigSnapshotService.HasSnapshot(scriptId, userName));
            File.WriteAllText(Path.Combine(ConfigPaths.StoreDir(scriptId, userName), "config.json"), "{}");
            Assert.True(ConfigSnapshotService.HasSnapshot(scriptId, userName));
        }
        finally
        {
