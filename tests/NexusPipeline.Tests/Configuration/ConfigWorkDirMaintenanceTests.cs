...
public class ConfigWorkDirMaintenanceTests
{
    ...
    public void ConfigSwapPaths_WorkLayout_PinsConsolidatedPaths()
    {
        ...
        Assert.Equal(Path.Combine(userRoot, "store"), ConfigPaths.StoreDir(scriptId, userName));
        Assert.Equal(Path.Combine(userRoot, "store-meta.json"), ConfigPaths.StoreMetadataPath(scriptId, userName));
        Assert.Equal(Path.Combine(work, "original"), ConfigPaths.CacheDir(scriptId, userName));
        Assert.Equal(Path.Combine(work, "edit-isolation"), ConfigPaths.EditIsolationDir(scriptId, userName));
        Assert.Equal(Path.Combine(work, "script"), ConfigPaths.ScriptDir(scriptId, userName));
        Assert.Equal(Path.Combine(work, "swap-backup"), ConfigPaths.ReplaceBackupDir(scriptId, userName));
        // 无用户兜底同样收敛到 work/
        Assert.Equal(Path.Combine(ScriptRoot(scriptId), "work", "script"), ConfigPaths.ScriptDir(scriptId, null));
        Assert.Equal(Path.Combine(ScriptRoot(scriptId), "work", "swap-backup"), ConfigPaths.ReplaceBackupDir(scriptId, null));
    }
    ...
    public void UntrackedEditIsolationResidue_IsPreservedWithoutGuessing()
    {
        ...
        const string userName = "user-a";
        string isolationRoot = ConfigPaths.EditIsolationDir(scriptId, userName);
        try
        {
            Directory.CreateDirectory(isolationRoot);
            File.WriteAllText(Path.Combine(isolationRoot, "orphan"), "保留现场");
            ConfigRecoveryService.RecoverInterrupted(
            [
                new NexusUser
                {
                    Id = userName,
    ...
    public void UntrackedExtraConfigResidue_IsPreservedWithoutGuessing()
    {
        ...
        const string userName = "user-a";
        string originalExtraRoot = ConfigPaths.OriginalExtraRoot(scriptId, userName);
        try
        {
