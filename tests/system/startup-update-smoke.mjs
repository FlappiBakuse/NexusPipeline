  ...
  const server = http.createServer((request, response) => {
    if (request.url === "/releases") {
      ...
      const release = {
        ...
        prerelease: true,
        tag_name: "v0.12.10",
        name: "v0.12.10 test release",
        body: "startup update regression fixture",
        assets: [
          {
            name: "NexusPipeline-v0.12.10-win-x64.zip",
            browser_download_url: `http://127.0.0.1:${server.address()?.port || 0}/NexusPipeline-v0.12.10-win-x64.zip`,
          },
          {
            name: "NexusPipeline-v0.12.10-win-x64.zip.sha256",
            browser_download_url: `http://127.0.0.1:${server.address()?.port || 0}/NexusPipeline-v0.12.10-win-x64.zip.sha256`,
          },
        ],
      };
...
test("启动自动检查：开启时无需手动调用且只请求一次更新源", { skip: skipReason, concurrency: false }, async () => {
  ...
  try {
    ...
    assert.equal(status.latest, "0.12.10");
  } finally {
