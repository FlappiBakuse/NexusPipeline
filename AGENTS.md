...
- 可见交互控件使用 `wwwroot/core/controls.js` 的 NexusPipeline 自定义控件层、`data-action` 和稳定 `data-testid`；隐藏文件载体、系统取色载体与语义滑块保留为底层无障碍/浏览器接口。禁止 inline 事件、内联 style、散落颜色字面量和需要打包的依赖。
