...
- **判断脚本契约（v0.4.0+）**：
  - **输入 JSON**：脚本字段 + 用户 + `config`（运行时生效配置 ConfigPath，只读）与 `script`（`data/{脚本Id}/{UserId}/script`，可读写；无用户兜底 `data/{脚本Id}/script`）全递归文件清单 + `scriptDir` + **本次尝试日志段**（v0.5.2+：判断脚本输入按尝试切片，只读当前尝试内容，上次尝试的失败/成功行不跨尝试污染判定；v0.4.3+：超过 4MB 仅提供尾部并置 `logTruncated`=true，防大日志拖垮内置引擎）。
...
- **插队替换配置（v0.4.0+）**：
  ...
  - **重试衔接（v0.7.8）**：替换供重试轮使用，本次尝试失败后先保存到运行期 `retry-store`，恢复 original 真实现场，再重新执行完整配置交换加载下一轮；用户永久 `store` 只由自动更新配置收尾同步决定。首次替换前备份到 `data/{脚本Id}/{UserId}/swap-backup`，`.meta` 记录 configPath 与新增文件清单，还原时删除新增文件；config 为单文件时 replaceConfigs 项须等于该文件名（忽略大小写）方可替换，其余目标拒绝。
...
- **编辑会话隐藏机制（v0.6.0+）**：专项脚本 + config 为单文件时，start 将 config 同目录下其他 `*.json` 配置（如 BetterGI 自带「默认配置.json」）移入 `data/{脚本Id}/{UserId}/edit-hidden`（hideDir），使编辑目标成为唯一可选配置；done/cancel 恢复，崩溃残留由下次 start 幂等恢复（`RecoverInterrupted` 亦恢复 hideDir）。
...
- **数据目录命名（v0.6.0+）**：`data/{脚本Id}/{UserId}/` 下为 `store/`（配置快照）、`store.previous/`（上一份完整快照）、`store.tmp`（同步临时目录）、`retry-store/`（当前运行重试快照）、`original/`（原配置暂存）、`script/`、`swap-backup/`、`edit-hidden/`、`.session`；`Name` 只用于展示，配置写入、运行和恢复均使用 `UserId`。启动恢复按当前 `NexusUser.Id + binding.ScriptInstanceId` 建立精确目录集合并迁移现场；历史用户名目录作为惰性遗留保留，不参与读写、恢复或递归清理。
