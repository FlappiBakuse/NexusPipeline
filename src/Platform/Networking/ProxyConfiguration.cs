...
/// <summary>从已投影的代理参数构造一次性 HTTP handler。</summary>
internal sealed record ProxyConfiguration(
    ...
    string Password)
{
    public static ProxyConfiguration FromOptions(OutboundProxyOptions options)
    {
        ArgumentNullException.ThrowIfNull(options);
        string mode = (options.Mode ?? "none").Trim().ToLowerInvariant();
        ...
        {
            if (!Uri.TryCreate(options.Url.Trim(), UriKind.Absolute, out Uri? parsed)
