    ...
    public async Task<RunScreenshot?> CaptureAsync(int attemptNumber, string trigger, CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                ...
                DateTimeOffset capturedAt = captured.CapturedAt ?? DateTimeOffset.Now;
                ...
                if (captured.FromCache)
                {
                    string age = captured.CacheAge.HasValue
                        ? $"，缓存年龄 {captured.CacheAge.Value.TotalMilliseconds:0} ms"
                        : "";
                    Logger.Info($"[截图] {FormatTrigger(trigger)}使用最近缓存帧{age}。");
                }
                lock (_sync)
                {
                    if (_disposed)
