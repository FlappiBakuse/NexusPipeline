...
/// <summary>Judge 专用单飞 worker，保留清晰的运行时边界供 AttemptMonitor 使用。</summary>
internal sealed class SingleFlightJudgeWorker : IAsyncDisposable
{
    private readonly SingleFlightWorker<JudgeSnapshot, JudgeWorkerResult> _worker;

    public SingleFlightJudgeWorker(Func<JudgeSnapshot, CancellationToken, Task<JudgeWorkerResult>> handler)
    {
        _worker = new SingleFlightWorker<JudgeSnapshot, JudgeWorkerResult>(handler);
    }

    public bool TryStart(JudgeSnapshot snapshot) => _worker.TryStart(snapshot);

    public bool TryTakeCompleted(out JudgeWorkerResult result, out Exception? error)
        => _worker.TryTakeCompleted(out result, out error);
...
/// <summary>配置同步单飞 worker。同步动作完成后才允许 Attempt 收尾进入 FinalizeRun。</summary>
internal sealed class ConfigSyncWorker : IAsyncDisposable
{
    private readonly SingleFlightWorker<ConfigSyncRequest, bool> _worker;

    public ConfigSyncWorker(Action<ConfigSyncRequest> sync)
    {
        ArgumentNullException.ThrowIfNull(sync);
        _worker = new SingleFlightWorker<ConfigSyncRequest, bool>((request, token) =>
        {
    ...
    public bool TryStart(ConfigSyncRequest request) => _worker.TryStart(request);

    public bool TryTakeCompleted(out bool result, out Exception? error)
        => _worker.TryTakeCompleted(out result, out error);
