...
internal sealed class NotificationDispatcher : INotificationService
{
    ...
    public async Task NotifyScriptAsync(ScriptInstance script, RunRecord record)
    {
        ...
        await SendTextAsync(settings, NotificationFormatter.Script(script, record), "脚本").ConfigureAwait(false);
    }
    ...
    public async Task NotifyQueueAsync(DispatchQueue queue, List<RunRecord> records)
    {
        ...
        await SendTextAsync(settings, NotificationFormatter.Queue(queue, records), "队列").ConfigureAwait(false);
    }
    ...
    public async ValueTask SendPluginAsync(PluginNotification notification, CancellationToken cancellationToken)
    {
        ...
        await SendTextAsync(settings, body, "插件").WaitAsync(_channelTimeout, cancellationToken).ConfigureAwait(false);
    }

    private async Task SendTextAsync(AppSettings settings, string text, string kind)
    {
        try
        {
            await NotifySender.SendAsync(settings, text).WaitAsync(_channelTimeout).ConfigureAwait(false);
        }
    ...
    private static bool HasChannel(AppSettings settings)
    {
        ...
        (bool smtpOk, _) = SmtpSender.Status(settings);
