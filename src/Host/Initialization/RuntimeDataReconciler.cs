...
/// 读取由 HostCompositionRoot.ReloadData 完成；本类负责当前数据完整性修复、名称消歧和必要落盘。
/// </summary>
internal static class RuntimeDataReconciler
{
    ...
    private static void NormalizeEntityNames(AutomationDefinitionMutationContext state)
    {
        ...
        if (scriptChanges.Count > 0)
        {
            try
            {
                ScriptDefinitionStore.SaveScripts(normalizedScripts);
                state.Scripts.Clear();
        ...
        if (queueChanges.Count > 0)
        {
            try
            {
                QueueDefinitionStore.SaveQueues(normalizedQueues);
                state.Queues.Clear();
        ...
        if (userChanges.Count > 0)
        {
            try
            {
                UserDefinitionStore.SaveUsers(normalizedUsers);
                state.Users.Clear();
