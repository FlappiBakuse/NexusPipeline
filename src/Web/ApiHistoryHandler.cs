...
internal static class ApiHistoryHandler
{
    public static async Task Handle(HttpListenerContext context, string method, string[] seg)
    {
        ...
        if (seg.Length == 2 && seg[1].Equals("summary", StringComparison.OrdinalIgnoreCase))
        {
            ...
            HistorySummary summary = RuntimeContext.Instance.History.Summarize(
                summaryStart,
                summaryEnd,
                summaryScriptId,
                summaryQueueId,
        ...
        if (seg.Length == 2 && seg[1].Equals("users", StringComparison.OrdinalIgnoreCase))
        {
            ...
            List<HistoryUserSummary> users = RuntimeContext.Instance.History.QueryUsers(day, scriptId, queueId, status);
        ...
        if (seg.Length == 2 && seg[1].ToLowerInvariant() == "detail")
        {
            ...
            System.Text.Json.Nodes.JsonObject recordView = HistoryService.ToView(record);
            ...
            foreach (RunAttempt attempt in record.AttemptDetails)
            {
                ...
                attemptLogs.Add(new
                {
                    ...
                    durationMs = HistoryService.DurationMilliseconds(attempt),
            ...
            await HttpHelper.WriteJsonAsync(context, new
            {
                ...
                records = dayRecords
                    ...
                    .Select(record =>
                    {
                        ...
                        return HistoryService.ToView(localized);
                    })
                    .ToList(),
            }).ConfigureAwait(false);
            return;
        ...
        out string? error)
    {
        ...
        status = HistoryService.NormalizeStatus(request.QueryString["status"]);
        if (userKey is not null && !HistoryService.IsValidUserKey(userKey))
        {
            error = "history_invalid_user_key";
            return false;
        }
        if (!HistoryService.IsValidStatus(status))
        {
