...
internal static class ConfigStoreTransaction
{
    ...
    {
        if (File.Exists(ConfigPaths.StoreTransactionBlockedPath(scriptId, userKey)))
        {
            throw new IOException($"配置快照事务已被阻断，需人工核查后解除：{ConfigPaths.StoreTransactionBlockedPath(scriptId, userKey)}");
        }
...
internal static class ConfigStoreTransactionRecovery
{
    public static void Recover(string scriptId, string userKey)
    {
        ...
        string manifestPath = ConfigPaths.StoreTransactionManifestPath(scriptId, userKey);
        ...
        string commitPath = ConfigPaths.StoreTransactionCommitPath(scriptId, userKey);
    ...
    private static void Rollback(string scriptId, string userKey, ConfigStoreTransactionManifest manifest)
    {
        ...
        string metadataPath = ConfigPaths.StoreMetadataPath(scriptId, userKey);
    ...
    private static IOException Quarantine(string transactionDir, string scriptId, string userKey, string reason)
    {
        ...
        try
        {
            JsonUtil.WriteAtomic(
                ConfigPaths.StoreTransactionBlockedPath(scriptId, userKey),
