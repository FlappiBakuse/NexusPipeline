...
internal static class ConfigWorkDirMaintenance
{
    ...
    public static void SweepIdleWorkDirs()
    {
        ...
        foreach (string scriptDir in Directory.GetDirectories(AppPaths.DataDir))
        {
            ...
            {
                if (string.Equals(Path.GetFileName(userDir), ConfigPaths.WorkDirName, StringComparison.OrdinalIgnoreCase))
                {
                    continue;
                }
    ...
    {
        string workDir = Path.Combine(ownerDir, ConfigPaths.WorkDirName);
