...
internal sealed class ConfigSwapRecovery
{
    ...
    {
        string dir = ConfigPaths.ReplaceBackupDir(scriptId, userName);
    ...
    private bool TryFinalizePendingInput(string scriptId, string userName, ConfigSessionMark mark)
    {
        ...
        if (mark.PendingConfigInput is null
            ...
            || !Directory.Exists(ConfigPaths.StoreDir(scriptId, userName))
            || !Directory.EnumerateFileSystemEntries(ConfigPaths.StoreDir(scriptId, userName)).Any()
            || metadata is null
