...
internal sealed class SystemActionExecutor
{
    ...
    private readonly IApplicationExitRequest? _exitRequest;

    public SystemActionExecutor(
        ExecutionStateStore state,
        RealtimeEventBus? realtime = null,
        IApplicationExitRequest? exitRequest = null)
    {
        _state = state;
        _realtime = realtime;
        ...
        try
        {
            switch (pending.Action)
            {
                ...
                case "exit":
                    ...
                    {
                        PublishSystemAction("armed", pending);
                        StartDelay(pending, () => SystemActions.ExitApp(_exitRequest), TimeSpan.Zero);
                    }
                    break;
                default:
