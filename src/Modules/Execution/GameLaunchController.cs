...
internal sealed class GameLaunchController
{
    ...
    public async Task<RunAttemptResult?> LaunchAsync()
    {
        ...
        if (!ExecutablePathRules.IsExecutable(_script.GameExe))
        {
        ...
        try
        {
            ...
            ProcessStartInfo gamePsi = SystemActions.BuildScriptStartInfo(
                _script.GameExe,
                gameWork,
                CommandLineTokenizer.SplitArgs(_script.GameArgs),
                noWindow: false,
    ...
    private async Task<RunAttemptResult?> LaunchEmulatorCoreAsync()
    {
        ...
        string[] startArgs = CommandLineTokenizer.SplitArgs(_script.GameArgs).ToArray();
