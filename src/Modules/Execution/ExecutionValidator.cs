...
/// 不创建运行状态，也不启动后台任务，避免 ExecutionDispatcher 同时承担验证和生命周期管理。
/// </summary>
internal sealed class ExecutionValidator
{
    ...
    public void ValidateQueueStartSnapshot(DispatchQueue queue, IReadOnlyList<ScriptInstance> scripts)
    {
        ...
        string? mixError = QueueCompositionPolicy.CheckQueueMix(scripts, queue);
    ...
    {
        return QueueCompositionPolicy.CheckQueueMix(_scripts.Snapshot(), queue);
    }
