...
internal sealed class SettingsCommands
{
    ...
    private readonly ISettingsMutationGate _mutationGate;
    private readonly ISettingsChangedEffects _effects;

    internal SettingsCommands(
        SettingsState state,
        ISettingsMutationGate mutationGate,
        ISettingsChangedEffects effects)
    {
        _state = state;
    ...
    {
        AppSettings candidate = _state.Current;
        ...
        try
        {
            bool admitted = _mutationGate.TryExecute(() =>
            {
                lock (_state.MutationLock)
                {
                    ...
                    foreach (KeyValuePair<string, JsonNode?> pair in patch)
                    {
                        ...
                        {
                            bindError = "插件仓库通道只能在配置文件中设置，并在重启后生效";
                            break;
                        }
            ...
            _effects.Apply(previous!, current);
            Audit.Log(
                source,
                "保存设置",
