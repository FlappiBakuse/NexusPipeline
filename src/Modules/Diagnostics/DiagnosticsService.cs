...
internal sealed class DiagnosticsService
{
    ...
    private readonly IControlPlaneStatusReader _controlPlane;
    ...
    public DiagnosticsService(
        ...
        IControlPlaneStatusReader controlPlane)
    {
        _settings = settings;
        _scripts = scripts;
        _plugins = plugins;
    ...
    {
        ListenerStatus? server = _controlPlane.Web;
    ...
    private DiagnosticCheck CheckMcpListener()
    {
        ...
        ListenerStatus? host = _controlPlane.Mcp;
        return host is { IsRunning: true }
    ...
    private DiagnosticCheck CheckConfigRecovery()
    {
        try
        {
            int editSessions = ConfigEditSessionRegistry.EditSessions.Count;
            ...
            int workDirs = Directory.Exists(AppPaths.DataDir)
                ? Directory.EnumerateDirectories(AppPaths.DataDir, ConfigPaths.WorkDirName, SearchOption.AllDirectories).Count()
                : 0;
            if (blocked > 0)
            {
                return Fail(
