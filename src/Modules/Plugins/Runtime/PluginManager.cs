...
internal sealed class PluginManager : IPluginCapabilityResolver, IPluginAvailability, IUserRunStartingPublisher, IEmulatorSupportProviderResolver, IPluginExecutionProviderResolver
{
    ...
    internal PluginManager(
        ...
        IPluginConfigurationMutationGate configurationGate,
    ...
    public bool SetEnabled(string name, bool enabled, string source, out string? failureCode)
    {
        ...
        bool changed = _configurationGate.TryExecute(() =>
        {
        ...
        }, out string? gateFailureCode);
        if (!changed)
        {
            failureCode = gateFailureCode ?? "host_maintenance";
            return false;
        }
        ...
        out string? filePath)
    {
        ...
        string? pluginDirectory = _managedPlugins.FirstOrDefault(plugin =>
