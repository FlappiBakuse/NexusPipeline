...
internal sealed class PluginPackageService
{
    ...
    public PluginPackageService(OutboundHttpClientProvider outbound, HostVersionInfo? hostVersion = null)
    {
        _outbound = outbound;
    ...
    public Task<PluginPendingOperation> StageAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        if (candidate.FrozenEntry is null)
        {
            throw new PluginRepositoryException("candidate_invalid", "插件包候选缺少冻结 catalog entry");
        }
        return StageCoreAsync(candidate.FrozenEntry, action, cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        string? urlError = PluginRepositoryCatalog.ValidatePackageUrl(
            ...
            entry.Version,
            entry.Sha256,
            PluginRepositorySourceContext.ForChannel(entry.Channel));
    ...
    private static void ValidateManifest(PluginCatalogEntry entry, string pluginDir)
    {
        try
        {
            ...
            {
                if (!SpecializedPluginContract.TryValidatePayload(pluginDir, out string? specializedError))
                {
                    throw new PluginRepositoryException("manifest_invalid", specializedError ?? "数据化专项插件载荷不符合 Host 契约");
                }
