...
internal sealed class PluginStoreProjector
{
        ...
        string? error)
    {
        ...
        foreach (PluginCatalogEntry entry in catalog.Plugins)
        {
            ...
            PluginOwnership? owner = ownership.TryGetValue(entry.Name, out PluginOwnership? foundOwner)
                ? foundOwner
                : null;
            ...
            bool updateAvailable = decision.CanStage
                && decision.Kind is (
                    PluginUpdateDecisionKind.Upgrade
                    or PluginUpdateDecisionKind.RefreshPreview
                    or PluginUpdateDecisionKind.ReplacePreviewWithStable);
                ...
                entry.Changelog)
            {
                ...
                UpdateDecisionKind = decision.Kind,
                UpdateDecisionReasonCode = decision.ReasonCode,
                Candidate = updateAvailable
                    ? PluginPackageCandidate.FromEntry(entry, _sourceContext)
                    : null,
            });
        }
