...
internal sealed class PluginManifest
{
    ...
    public static bool TryLoad(string pluginDir, out PluginManifest? manifest, out string? error)
    {
        ...
        try
        {
            ...
            {
                error = "此插件声明已退役的 configValidator；请升级到支持 taskProtocol 配置诊断的版本，或卸载此插件。现有用户配置不会被修改。";
                return false;
            }
            ...
            if (result.Kind == "data-specialized"
                && !SpecializedPluginContract.TryValidatePayload(pluginDir, out string? specializedError))
            {
                error = specializedError ?? "data-specialized 插件载荷不符合 Host 契约";
                return false;
            }
            manifest = result;
            return true;
