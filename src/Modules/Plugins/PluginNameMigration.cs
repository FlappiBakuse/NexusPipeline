...
internal static class PluginNameMigration
{
    ...
    internal static void Apply(
        ...
        Action<IReadOnlyList<ScriptInstance>> replaceScripts)
    {
        MigrateSettings(settings, replaceSettings);
        MigrateScripts(scripts, replaceScripts);
        MigratePluginFiles();
    }

    private static void MigrateSettings(AppSettings settings, Action<AppSettings> replaceSettings)
    {
    ...
    private static void MigrateScripts(
        ...
        Action<IReadOnlyList<ScriptInstance>> replaceScripts)
    {
        List<ScriptInstance> migrated = scripts.Select(script => script.Clone()).ToList();
        int changed = 0;
        ...
        try
        {
            ScriptDefinitionStore.SaveScripts(migrated);
            replaceScripts(migrated);
