...
internal sealed class McpReadOnlyTools
{
        ...
        string? userName = null)
    {
        ...
        return !script.Succeeded
            ...
            : McpToolResult.Success(_context.ExecutionExplain
    ...
    public CallToolResult ListUsers()
    {
        ...
        return McpToolResult.Success(_context.Users
            .Select(user => McpViews.User(user, queues, _context.Scheduler))
            .ToList());
    }
    ...
    public CallToolResult ListQueues()
    {
        return McpToolResult.Success(_context.Queues
            .Select(queue => McpViews.Queue(queue, _context.Scheduler))
            .ToList());
    }
    ...
    {
        return McpToolResult.Success(_context.Center.Active
        ...
        string runId)
    {
        ...
        RunningExecution? execution = _context.Center.FindAny(runId.Trim());
        ...
        string? status = null)
    {
        ...
        string? normalizedStatus = RunHistoryService.NormalizeStatus(status);
        if (!RunHistoryService.IsValidStatus(normalizedStatus))
        {
            return McpToolResult.Failure(
        ...
        List<RunRecord> records = _context.History.Query(
        ...
        {
            total = records.Count,
            records = records.Skip(offset).Take(limit).Select(RunHistoryService.ToView).ToList(),
        });
    }
    ...
    {
        return McpToolResult.Success(_context.Plugins.GetLocalizedPluginManagementViews(LocaleContext.Current));
    }
    ...
    private async Task<CallToolResult> ReadPluginStoreAsync(bool forceRefresh)
    {
        try
        {
            PluginStoreSnapshot snapshot = await _context.PluginRepository
