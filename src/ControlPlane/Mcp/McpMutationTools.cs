...
internal sealed class McpMutationTools
{
        ...
        string? userReference = null)
    {
        ...
        try
        {
            RunningExecution execution = _context.Center.StartScript(
                script.Value!.Id,
                "mcp",
                Audit.Mcp,
                userName);
        ...
        string queueReference)
    {
        ...
        try
        {
            RunningExecution execution = _context.Center.StartQueue(
                queue.Value!.Id,
                "mcp",
                Audit.Mcp);
            return McpToolResult.Success(new
        ...
        string runId)
    {
        ...
        if (_context.Center.Find(runId.Trim()) is null)
        {
            return McpToolResult.Failure(
                "not_found",
                $"未找到活动运行：{runId}",
        ...
        try
        {
            _context.Center.Cancel(runId.Trim(), Audit.Mcp);
    ...
    {
        bool canceled = _context.Center.CancelSystemAction(Audit.Mcp);
        ...
        McpScriptInput input)
    {
        ...
        return McpToolResult.From(
            _context.ScriptCommands.Create(input.ToModel(), Audit.Mcp),
        ...
        McpScriptInput input)
    {
        ...
        return McpToolResult.From(
            _context.ScriptCommands.Update(target.Value!.Id, input.ToModel(), Audit.Mcp),
            value => value is null ? null : McpViews.Script(value, _context.Users));
    }
        ...
        McpUserInput input)
    {
        ...
        OperationResult<NexusUser> result = _context.UserCommands.Create(
            input.Name,
            input.Remark,
            Audit.Mcp);
        return McpToolResult.From(
            ...
            value => value is null
                ? null
                : McpViews.User(value, _context.Queues, _context.Scheduler));
    }
        ...
        McpBindingInput input)
    {
        ...
        OperationResult<UserScriptBinding> result = _context.UserCommands.AddBinding(
            user.Value!.Id,
        ...
        McpBindingInput input)
    {
        ...
        OperationResult<UserScriptBinding> result = _context.UserCommands.UpdateBinding(
            user.Value!.Id,
            script.Value!.Id,
