...
internal static class ApiPluginContributionsHandler
{
        ...
        PluginUserGlobalSettingsService userGlobalSettings)
    {
        ...
        {
            await QueryUiAsync(context, body, plugins).ConfigureAwait(false);
            return;
        }
        if (TryParseUiActionRoute(method, seg, out string actionPluginName, out string actionContributionId, out string action))
        {
            await RunUiActionAsync(context, actionPluginName, actionContributionId, action, body, plugins).ConfigureAwait(false);
            return;
        }
        if (TryParseUiSaveRoute(method, seg, out string savePluginName, out string saveContributionId))
        {
            await SaveUiAsync(context, savePluginName, saveContributionId, body, plugins).ConfigureAwait(false);
            return;
        }
        if (TryParseUserListBadgesRoute(method, seg))
        {
            await ReadUserListBadgesAsync(context, plugins, userQueries).ConfigureAwait(false);
            return;
        }
        ...
        if (userQueries.Find(userId) is null)
        {
        ...
        if (method == "GET")
        {
            await ReadAsync(context, userId, userGlobalSettings).ConfigureAwait(false);
            return;
        }
        if (method == "PUT")
        {
            await SaveAsync(
                ...
                contributionId,
                body,
                userGlobalSettings).ConfigureAwait(false);
            return;
        }
        await HttpHelper.MethodNotAllowedAsync(context).ConfigureAwait(false);
    }
    ...
    private static async Task QueryUiAsync(
        HttpListenerContext context,
        string body,
        PluginManager plugins)
    {
    ...
    private static async Task ReadAsync(
        ...
        PluginUserGlobalSettingsService userGlobalSettings)
    {
        OperationResult<IReadOnlyList<PluginUserGlobalSettingsView>> result = await userGlobalSettings
            .ReadAsync(userId)
            .ConfigureAwait(false);
        if (!result.Succeeded)
        {
    ...
    private static async Task ReadUserListBadgesAsync(
        HttpListenerContext context,
        PluginManager plugins,
        UserQueries userQueries)
    {
        IReadOnlyList<PluginUserListBadgeRegistration> contributions = plugins.UserListBadgeContributions;
        IReadOnlyList<UserReadModel> users = userQueries.List();
        ...
        PluginUserGlobalSettingsService userGlobalSettings)
    {
        ...
        OperationResult<bool> result = await userGlobalSettings
