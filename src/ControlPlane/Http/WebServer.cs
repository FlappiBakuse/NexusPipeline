...
internal sealed class WebServer : IDisposable
{
    ...
    private static readonly IReadOnlyDictionary<string, ApiRouteCatalog.RouteDefinition> RouteMetadata =
        ApiRouteCatalog.Build(typeof(WebServer).Assembly);

    private readonly IHttpRouteBindings _routeBindings;
    ...
    {
        _routeBindings = routeBindings ?? throw new ArgumentNullException(nameof(routeBindings));
    ...
    internal static IReadOnlyList<string> RegisteredApiRouteNames =>
        RouteMetadata.Keys.OrderBy(name => name, StringComparer.OrdinalIgnoreCase).ToArray();
    ...
    {
        if (RouteMetadata.TryGetValue(name, out ApiRouteCatalog.RouteDefinition? route))
        {
    ...
    private static Task InvokeRouteAsync(
        ApiRouteCatalog.BoundRoute route,
        HttpListenerContext context,
        string methodName,
        string[] seg,
        string body,
        CancellationToken cancellationToken,
        IHttpRouteBindings routeBindings)
    {
        MethodInfo mi = route.Definition.Method;
        ...
        for (int i = 2; i < parameters.Length; i++)
        {
            args[i] = parameters[i].ParameterType switch
            {
                ...
                var type when type == typeof(SettingsCommands) => routeBindings.SettingsCommands,
                var type when type == typeof(ScriptCommands) => routeBindings.ScriptCommands,
                var type when type == typeof(QueueCommands) => routeBindings.QueueCommands,
                var type when type == typeof(UserCommands) => routeBindings.UserCommands,
                var type when type == typeof(ConfigEditCommands) => routeBindings.ConfigEditCommands,
                var type when type == typeof(ScriptQueries) => routeBindings.ScriptQueries,
                var type when type == typeof(ScriptSaveValidation) => routeBindings.ScriptSaveValidation,
                var type when type == typeof(DiagnosticsService) => routeBindings.Diagnostics,
                var type when type == typeof(ExecutionDispatcher) => routeBindings.ExecutionDispatcher,
                var type when type == typeof(ExecutionExplainService) => routeBindings.ExecutionExplain,
                var type when type == typeof(RealtimeEventBus) => routeBindings.RealtimeEvents,
                var type when type == typeof(ExecutionPreviewService) => routeBindings.ExecutionPreview,
        ...
        object? result = mi.Invoke(route.Target, args);
    ...
    public void Stop()
    {
        ...
        try
        {
            if (LocalFileReader.Exists(AppPaths.WebPortPath))
            {
                LocalFileReader.Delete(AppPaths.WebPortPath);
            }
        }
        catch (Exception ex)
        {
    ...
    private async Task HandleAsync(HttpListenerContext context, CancellationToken token)
    {
        ...
        try
        {
            ...
            if (path.StartsWith("/plugin-assets/", StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (!_plugins.TryResolveFrontendAsset(
                        pluginName,
                        relativePath,
    ...
    private bool AuthorizeRequest(HttpListenerContext context, out string? detail)
    {
    ...
    private async Task HandleApiAsync(HttpListenerContext context, string method, string path, CancellationToken token)
    {
        string[] segments = path.Trim('/').Split('/', StringSplitOptions.RemoveEmptyEntries);
        ApiRouteCatalog.BoundRoute? route = segments.Length >= 2
            && segments[0].Equals("api", StringComparison.OrdinalIgnoreCase)
            && _routes.TryGetValue(segments[1], out ApiRouteCatalog.BoundRoute? resolved)
            ? resolved
            : null;
        string body = "";
        if (context.Request.HasEntityBody && route?.Definition.BodyMode != ApiBodyMode.Raw)
        {
            int maxBodyBytes = route is not null && route.Definition.MaxBodyBytes > 0
                ? route.Definition.MaxBodyBytes
                : MaxRequestBodyBytes;
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (_routes.TryGetValue(resource, out ApiRouteCatalog.BoundRoute? route))
        {
            await InvokeRouteAsync(
                ...
                body,
                cancellationToken,
                _routeBindings).ConfigureAwait(false);
            return;
        }
        await HttpHelper.NotFoundAsync(context).ConfigureAwait(false);
    }
