...
namespace NexusPipeline.Shared.Localization;
...
internal static class HostLocalization
{
    ...
    private static readonly IReadOnlyDictionary<string, string> English =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["exit.active_runs"] = "There are active runs. Wait for them to finish or cancel them first.",
            ["exit.blocked_title"] = "NexusPipeline cannot exit",
            ["exit.completion_delayed"] = "Completion exit request delayed: {reason}",
            ["exit.config_edit_sessions"] = "There are configuration edit sessions. Finish or cancel them first.",
            ["exit.pending_system_action"] = "There is a pending system action. Complete or cancel it first.",
            ["exit.request_rejected"] = "Exit request rejected: {reason}",
            ["exit.shutdown_blocked"] = "The host shutdown was refused because services are still active: {reason}",
            ["exit.unknown_reason"] = "NexusPipeline cannot exit right now. Try again later.",
            ["exit.waiting_for_safe_shutdown"] = "Waiting for runs or edit sessions to finish before stopping the host: {reason}",
            ["script.name_required"] = "Script name is required",
            ["startup.admin_required"] = "NexusPipeline must run as administrator because script programs require administrator access. This instance did not receive administrator privileges and will exit. Right-click and choose \"Run as administrator\", or verify that the requireAdministrator build is deployed.",
            ["startup.admin_title"] = "NexusPipeline requires administrator privileges",
            ["startup.limits_fatal"] = "A fatal limits configuration error prevented startup. Fix config/limits.json and try again.",
