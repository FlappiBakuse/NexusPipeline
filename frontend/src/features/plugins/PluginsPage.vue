...
async function runPluginAction(action: string) {
  const name = selected[activeTab.value];
  if (!name) return;
  try {
    ...
    toast(t("plugins.status.action_queued"));
