...
import { taskNameKeys, taskReasonKeys } from "../utils/taskLabels";
import type { TaskDefinition, TaskReport, TaskResult } from "../utils/taskTypes";
...
const name = computed(() => /^(march7th|zzzonedragon|baah|bettergi):/.test(props.task.id)
  ? t(`tasks.name.${taskNameKeys[props.task.name] || "custom"}`, {}, props.task.name)
  : props.task.name);
...
function reason(code: string) { return t(`tasks.reason.${taskReasonKeys[code] || code}`, {}, t('tasks.reason.other')); }
...
<template>
  <NxpCollapsibleCard class="task-card" :title="name" :expanded="expanded" :surface="depth % 2 === 0 ? 'secondary' : 'default'" :data-task-id="task.id" @toggle="expanded = $event">
    ...
    <div v-if="children.length" class="task-plan-children">
      <TaskPlanItem v-for="child in children" :key="child.id" :task="child" :tasks="tasks" :report="report" :focus-task-id="focusTaskId" :depth="depth + 1" :ancestors="[...ancestors, task.id]" />
    </div>
    ...
    <div v-for="attempt in report?.attemptReports || []" :key="attempt.attemptId" class="task-attempt" :data-attempt-id="attempt.attemptId" tabindex="-1">
      ...
      <template v-for="result in attempt.taskResults.filter(result => result.taskId === task.id)" :key="result.taskId">
        <p>{{ t(`tasks.status.${result.status}`) }} · {{ reason(result.reasonCode) }}</p>
      ...
      <p v-if="attempt.retryDecision" class="muted">{{ t('tasks.retry') }} · {{ reason(attempt.retryDecision.reasonCode) }}</p>
    </div>
  </NxpCollapsibleCard>
</template>
