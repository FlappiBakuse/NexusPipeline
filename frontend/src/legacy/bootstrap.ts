...
import { getLocale, loadLocale } from "@legacy/core/i18n.js";
import { ensureAccessToken } from "./event-bridge";
...
async function updateLocalAddress() {
  ...
  try {
    ...
    element.textContent = port ? `${location.hostname}:${port}` : "NexusPipeline";
  } catch {
    element.textContent = "NexusPipeline";
  }
}
