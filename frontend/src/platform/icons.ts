...
/** 宿主与插件桥接共用的图标标记；类名按调用方场景传入。 */
export function renderIcon(name: string, className = "icon"): string {
  return `<svg class="${className}" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${iconPath(name)}</svg>`;
}
