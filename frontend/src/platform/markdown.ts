...
function placeholder(values: string[], html: string): string {
  const index = values.push(html) - 1;
  return `\u0000${index}\u0000`;
}

function restore(value: string, values: string[]): string {
  return value.replace(/\u0000(\d+)\u0000/g, (_, index) => values[Number(index)] || "");
}

function inlineMarkup(source: string): string {
  const values: string[] = [];
...
function listMarkup(lines: string[], start: number): { html: string; next: number } {
