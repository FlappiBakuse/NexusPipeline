...
<template>
  <div v-if="props.open" class="nxp-modal" role="dialog" aria-modal="true" :aria-label="props.title || undefined">
    ...
    <section class="nxp-modal-panel">
      <header v-if="title || closeable" class="nxp-modal-header"><h2 v-if="title">{{ title }}</h2><button v-if="closeable" type="button" aria-label="关闭" @click="emit('close')">×</button></header>
...
.nxp-modal-header button { min-width: 36px; min-height: 36px; border: 0; background: transparent; color: var(--nx-color-text); font-size: 22px; cursor: pointer; }
