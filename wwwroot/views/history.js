...
export async function pageHistory(token) {
  ...
  const tableRows = records.map(record => `<tr><td>${esc(fmtTime(record.startTime))}</td><td>${esc(scriptName(record.scriptInstanceId))}</td><td>${esc(queueName(record.queueId)) || "-"}</td><td>${record.mode === "auto" ? "自动" : "手动"}</td><td>${statusBadge(finalStatusOf(record))}</td><td class="ops"><button class="sm ghost" type="button" data-action="history-detail" data-id="${esc(record.id)}">查看详情</button></td></tr>`).join("");
  const mobileRecords = records.map(record => `<article class="history-record" data-testid="history-record"><div class="history-record-head"><strong>${esc(scriptName(record.scriptInstanceId))}</strong>${statusBadge(finalStatusOf(record))}</div><div class="history-record-meta"><span>${esc(fmtTime(record.startTime))}</span></div><div class="history-record-meta"><span>${record.mode === "auto" ? "自动运行" : "手动运行"}</span><span>${esc(queueName(record.queueId) || "独立运行")}</span></div><div class="history-record-actions"><button class="sm ghost" type="button" data-action="history-detail" data-id="${esc(record.id)}">查看详情</button></div></article>`).join("");
