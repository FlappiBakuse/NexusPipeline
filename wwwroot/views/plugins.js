...
export async function pagePlugins(token) {
  ...
  render(pageHeader("插件", "插件", "管理通知推送和本地扩展能力。") + `<section class="card table-surface"><div class="table-scroll"><table class="data-table plugins-table"><thead><tr><th scope="col">名称</th><th scope="col">类型</th><th scope="col">版本</th><th scope="col">说明</th><th scope="col">状态</th><th scope="col">操作</th></tr></thead><tbody>${plugins.map(plugin => `<tr><td class="plugin-name-cell"><div class="plugin-name-content"><strong class="plugin-name-scroll" tabindex="0" title="${esc(plugin.displayName)}"><span class="plugin-name-scroll-inner">${esc(plugin.displayName)}</span></strong>${plugin.isBuiltIn ? ' <span class="badge muted">内置</span>' : ""}</div></td><td><span class="badge ${plugin.kind === "specialized" ? "blue" : "muted"}">${plugin.kind === "specialized" ? "专用" : "通用"}</span></td><td>${esc(plugin.version)}</td><td class="muted">${esc(plugin.description)}</td><td>${plugin.enabled ? '<span class="badge ok">已启用</span>' : '<span class="badge muted">已禁用</span>'}</td><td class="ops"><div class="table-actions">${plugin.name === "notify" ? '<button class="sm ghost" type="button" data-action="plugin-config" data-name="notify">配置</button>' : ""}<button class="sm ghost" type="button" data-action="toggle-plugin" data-name="${esc(plugin.name)}" data-enabled="${!plugin.enabled}">${plugin.enabled ? "禁用" : "启用"}</button></div></td></tr>`).join("")}</tbody></table></div><p class="muted helper-copy plugin-helper">通用插件为程序添加能力；专用插件接管专项脚本实例配置（数据化目录：plugins 下的插件文件夹自动加载，可删可替换）。内置插件修改状态后需重启服务生效。</p></section>`);
}
