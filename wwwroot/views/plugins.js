...
export async function pagePlugins(token) {
  ...
  const groupMarkup = groups.map(group => `<section class="plugin-group"><div class="plugin-group-heading"><h3>${group.label}</h3><span>${group.items.length} 项</span></div>${group.items.map(plugin => {
    const runtimeState = plugin.state || (plugin.runtimeEnabled ? "Active" : "Disabled");
    const runtimeLabel = runtimeState === "Active" ? (plugin.enabled ? "运行中" : "运行中 · 待重启") : runtimeState === "InitFailed" ? "初始化失败" : plugin.enabled ? "待重启" : "已禁用";
    const runtimeClass = runtimeState === "Active" ? "ok" : runtimeState === "InitFailed" ? "danger" : "muted";
    return `<article class="plugin-row"><div class="plugin-row-main"><strong class="plugin-name-scroll" tabindex="0" title="${esc(plugin.displayName)}"><span class="plugin-name-scroll-inner">${esc(plugin.displayName)}</span></strong><span class="muted">${esc(plugin.description || "本地扩展能力")} · ${esc(plugin.version)}</span>${plugin.error ? `<span class="field-error-message">${esc(plugin.error)}</span>` : ""}</div><span class="badge ${runtimeClass}">${runtimeLabel}</span><div class="plugin-row-action row-actions">${plugin.name === "notify" ? '<button class="tertiary" type="button" data-action="plugin-config" data-name="notify">配置</button>' : ""}<button class="tertiary" type="button" data-action="toggle-plugin" data-name="${esc(plugin.name)}" data-enabled="${!plugin.enabled}">${plugin.enabled ? "禁用" : "启用"}</button></div></article>`;
  }).join("")}</section>`).join("");
