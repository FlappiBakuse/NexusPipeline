...
export async function pagePluginConfig(name, token) {
  ...
  const body = `<section class="card"><div class="section-heading"><h3>配置信息</h3><span class="badge ${plugin?.enabled ? "ok" : "muted"}">${plugin?.enabled ? "已启用" : "已禁用"}</span></div><div class="stats-inline"><div><strong>${stats.enabledScripts ?? 0}</strong><span> 个启用通知的脚本实例</span></div><div><strong>${stats.enabledQueues ?? 0}</strong><span> 个启用通知的调度队列</span></div></div></section>
    <section class="card"><button class="panel-toggle" type="button" data-action="toggle-panel" data-panel="panel-wh" aria-expanded="true"><span class="panel-arrow" id="arrow-wh">▾</span><span class="panel-label">Webhook 通知</span><span class="badge ${settings.webhookEnabled ? "ok" : "muted"}">${settings.webhookEnabled ? "已启用" : "已禁用"}</span></button><div id="panel-wh" class="panel-body"><label class="check"><input id="st-wh-enabled" type="checkbox" ${settings.webhookEnabled ? "checked" : ""}><span>启用 Webhook 通知</span></label><div class="form-grid">${selectField("st-whtype", "Webhook 类型", settings.webhookType, [{ value: "feishu", label: "Feishu" }, { value: "dingtalk", label: "Dingtalk" }, { value: "wecom", label: "WeCom" }, { value: "slack", label: "Slack" }, { value: "discord", label: "Discord" }, { value: "generic", label: "Generic" }], 'data-action="toggle-generic-template"')} ${valueField("st-whtimeout", "超时秒数", settings.webhookTimeout || 30, "number", 'min="1"')}</div><label class="field-label" for="st-whurl">Webhook 地址 ${settings.webhookUrl ? '<span class="badge ok">已设置</span>' : ""}</label><div class="secret-row"><input id="st-whurl" type="password" placeholder="${settings.webhookUrl ? "（已设置，留空不变）" : "https://..."}"><button class="sm" type="button" data-action="save-secret" data-secret="webhookUrl" data-input="st-whurl">保存地址</button></div><label class="field-label" for="st-whsec">Webhook 签名密钥 ${settings.webhookSecret ? '<span class="badge ok">已设置</span>' : ""}</label><div class="secret-row"><input id="st-whsec" type="password" placeholder="${settings.webhookSecret ? "（已设置，留空不变）" : ""}"><button class="sm" type="button" data-action="save-secret" data-secret="webhookSecret" data-input="st-whsec">保存密钥</button></div><div id="st-whtpl-box" ${settings.webhookType === "generic" ? "" : "hidden"}><label class="field-label" for="st-whtpl">generic 自定义模板（JSON，{text} 为消息占位符）</label><textarea id="st-whtpl">${esc(settings.webhookTemplate)}</textarea></div></div>
