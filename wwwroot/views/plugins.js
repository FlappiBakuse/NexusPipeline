...
import { pageHeader, selectField, valueField } from "../core/forms.js";
...
export async function pagePlugins(token) {
  ...
  render(pageHeader("插件", "插件", "管理通知推送和本地扩展能力。") + `<section class="card"><div class="table-scroll"><table class="data-table"><thead><tr><th scope="col">名称</th><th scope="col">类型</th><th scope="col">版本</th><th scope="col">说明</th><th scope="col">状态</th><th scope="col">操作</th></tr></thead><tbody>${plugins.map(plugin => `<tr><td><strong>${esc(plugin.displayName)}</strong>${plugin.isBuiltIn ? ' <span class="badge muted">内置</span>' : ""}</td><td><span class="badge ${plugin.kind === "specialized" ? "blue" : "muted"}">${plugin.kind === "specialized" ? "专用" : "通用"}</span></td><td>${esc(plugin.version)}</td><td class="muted">${esc(plugin.description)}</td><td>${plugin.enabled ? '<span class="badge ok">已启用</span>' : '<span class="badge muted">已禁用</span>'}</td><td class="ops"><div class="table-actions">${plugin.name === "notify" ? '<button class="sm" type="button" data-action="plugin-config" data-name="notify">配置</button>' : ""}<button class="sm" type="button" data-action="toggle-plugin" data-name="${esc(plugin.name)}" data-enabled="${!plugin.enabled}">${plugin.enabled ? "禁用" : "启用"}</button></div></td></tr>`).join("")}</tbody></table></div><p class="muted helper-copy">通用插件为程序添加能力；专用插件接管专项脚本实例配置（数据化目录：plugins 下的插件文件夹自动加载，可删可替换）。内置插件修改状态后需重启服务生效。</p></section>`);
}
...
export async function pagePluginConfig(name, token) {
  ...
  render(`<header class="page-head"><div class="page-head-copy"><a class="back-link" href="#/plugins">${icon("arrowLeft")} 返回插件</a><div class="eyebrow">插件配置</div><div class="page-head-main"><h2>${esc(plugin?.displayName || name)} · 配置</h2></div><p class="page-kicker">Webhook 和 SMTP 的密钥会在服务端加密保存。</p></div></header>${renderedBody}`);
}
