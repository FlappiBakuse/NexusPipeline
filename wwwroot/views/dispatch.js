...
function logMarkup(record) {
  ...
  return entries.length
    ...
    : `<span class="run-log-empty">(${t("dispatch.no_log_output")})</span>`;
}
...
function runningMarkup(running) {
  if (!running.length) return `<div class="empty"><strong>${t("dispatch.no_tasks_are_running")}</strong>${t("dispatch.running.select_help")}</div>`;
...
function updateRunning(status) {
  ...
  if (head) head.textContent = `${t("common.running")} (${running.length})`;
  ...
  if (!running.length && !list.querySelector(".empty")) {
    ...
    emptyEl.innerHTML = `<strong>${t("dispatch.no_tasks_are_running")}</strong>${t("dispatch.running.select_help")}`;
...
function dispatchScriptOption(script) {
  ...
  const unavailableLabel = pluginStatus.missing ? t("common.plugin.unknown_label") : t("common.plugin.unavailable_label");
  ...
  render(pageHeader(t("dispatch.scheduler"), t("dispatch.scheduler"), t("dispatch.page.help")) + pluginSlotMarkup("dispatch.cards", "dispatch.cards") + `
    ...
    <section class="content-section" aria-labelledby="dispatch-run-heading"><div class="section-heading"><h3 id="dispatch-run-heading">${t("dispatch.start_one_run")}</h3><span class="muted">${t("dispatch.plan.target_help")}</span></div>
      <div class="dispatch-runbar">
        ...
        <div class="field" id="dc-queue-wrap" hidden><label class="field-label" for="dc-queue-trigger">${t("common.schedule_queues")}</label>${selectControlMarkup("dc-queue", "", [{ value: "", label: t("dispatch.plan.queue_hint") }, ...queues.map(queue => ({ value: queue.id, label: queue.name }))], "", t("common.schedule_queues"))}</div>
        <div class="control-action"><button id="dc-explain" class="ghost" type="button" data-action="explain-current" data-testid="dispatch-explain">${t("dispatch.check_run_plan")}</button><button id="dc-run" class="primary" type="button" data-action="dispatch-current" data-testid="dispatch-run">${t("dispatch.run_script")}</button></div>
      </div>
...
export async function dispatchScript() {
  ...
  if (!id) { toast(t("common.select.script_instance_label"), "error"); return; }
  ...
  try { await api("POST", "/api/dispatch/script", { scriptId: id, mode: "manual" }); toast(t("dispatch.run_started")); }
...
export function dispatchKindChange(target) {
  ...
  if (runButton) runButton.textContent = t(kind === "queue" ? "dispatch.run_queue" : "dispatch.run_script");
}
...
function explainUserStatus(user) {
  ...
  const label = status === "ready" ? t("dispatch.ready") : status === "skipped" ? t("dispatch.will_skip") : t("dispatch.blocked");
  const css = status === "ready" ? "ok" : status === "skipped" ? "blue" : "bad";
  const today = Number.isInteger(user?.successfulRunsToday)
    ? t("dispatch.plan.successful_today", { successful: user.successfulRunsToday, maximum: user.maxSuccessfulRunsPerDay > 0 ? user.maxSuccessfulRunsPerDay : t("common.unlimited") })
  ...
  return `<div class="execution-plan-row execution-plan-user-row" role="row">
    <div class="execution-plan-cell execution-plan-name" role="cell">${esc(user?.userName || t("dispatch.unnamed_user"))}</div>
...
function explainQueueClassLabel(queueClass) {
  ...
  const key = normalized === "emulatoronly" ? "dispatch.queue_class_emulator_only" : normalized === "emulator_only" ? "dispatch.queue_class_emulator_only" : normalized === "standard" ? "dispatch.queue_class_standard" : "";
  return key ? t(key) : String(queueClass || t("dispatch.not_tracked"));
}

function explainCompletionActionLabel(action) {
  const labels = {
    none: "common.no_action",
    exit: "common.exit_application",
    sleep: "common.sleep",
    reboot: "common.restart",
    shutdown: "common.shut_down",
  };
  const key = labels[String(action || "none").toLowerCase()];
  return key ? t(key) : String(action || t("common.no_action"));
}
...
function explainTaskRow(task, index) {
  ...
  return `<div class="execution-plan-row execution-plan-task-row" role="row">
    ...
    <div class="execution-plan-cell execution-plan-users" role="cell"><span class="badge blue">${esc(t("dispatch.summary.users", { count: task.userCount ?? 0 }))}</span></div>
  </div>`;
}

function explainPlanMarkup(result) {
  const failure = result?.admissionFailure;
  const status = result?.admissible ? t("dispatch.ready_to_run") : t("dispatch.cannot_start_now");
  ...
  return `<div class="execution-plan" data-testid="execution-explain-result" role="region" aria-label="${esc(t("dispatch.run_plan_check"))}">
    <section class="execution-plan-summary">
      <div class="execution-plan-summary-main"><span class="execution-plan-summary-label">${esc(t("dispatch.target"))}</span><strong>${esc(result?.targetName || "")}</strong><span class="badge ${statusClass}">${esc(status)}</span></div>
      <div class="execution-plan-summary-stat"><span class="k">${esc(t("common.task"))}</span><strong class="execution-plan-stat-value">${esc(t("common.unit.tasks", { count: totalTasks }))}</strong><span class="muted execution-plan-stat-subvalue">${esc(queueClass)}</span></div>
      <div class="execution-plan-summary-stat"><span class="k">${esc(t("common.completion_action"))}</span><strong class="execution-plan-stat-value">${esc(completionAction)}</strong></div>
    </section>
...
export function cancelRun(runId) {
  confirmModal(t("dispatch.cancel_run"), t("dispatch.cancel.confirm"), "confirm-cancel-run", { id: runId });
}

export async function confirmCancelRun(runId) {
  try { await api("POST", "/api/cancel", { runId }); closeModal(); toast(t("dispatch.cancellation_requested")); }
  catch (error) { toast(error.message, "error"); }
}
