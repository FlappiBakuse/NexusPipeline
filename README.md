...
- **脚本实例**：每个实例包含
  ...
  - 专用脚本实例：由已启用的专用插件（如 BetterGI）接管主程序/参数/配置/日志与完成标志，新建与编辑时自动适配，无需手动填写；
  ...
  - 完成标志：由专用插件自动提供（BetterGI=`一条龙和配置组任务结束`、March7thAssistant=`游戏终止：StarRail`、ZenlessZoneZeroOneDragon=`关闭游戏成功`）；通用脚本无完成标志时按「进程自行退出」判定成功（自定义完成标志功能重构中，暂不开放）。
- **调度队列**：链式运行脚本实例
