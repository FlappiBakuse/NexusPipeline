...
- **脚本实例**：每个实例包含
  ...
  - 游戏配置：运行前是否启动游戏（PC 客户端）、游戏路径、启动参数、启动后等待秒数（默认 30）；游戏配置卡在弹窗内常驻显示（不与复选框绑定），勾选「运行脚本前启动游戏」时游戏路径必填且必须为可执行文件；**运行结束后强制关闭游戏**（仅在选择「运行脚本前启动游戏」时可启用，未选择时禁用且默认关闭）；
  ...
  - 自定义完成标志：成功/失败关键字（每行一组，组内逗号分隔为 AND、换行之间为 OR；命中失败关键字立即终止本次尝试，命中成功关键字等待脚本退出判定成功）；或使用判断脚本（JavaScript 内置引擎 / Python 系统解释器，日志新增/阻塞周期/进程退出时触发，输入为脚本实例字段+用户+config（只读）与 script（可读写）目录文件清单+日志全文的 JSON（超过 4MB 仅提供尾部并置 logTruncated=true），脚本输出 `{"status":"success|failed","reason":"原因","notifyText":"可选","replaceConfigs":["相对script目录路径"]}` 判定结果，无输出视为继续运行）；判断脚本优先于关键字；脚本返回 `replaceConfigs` 可在失败后替换配置文件并自动重试（运行结束还原），返回的自定义通知文本可替换通知正文；
...
- **历史记录**：每个脚本实例/调度队列的完整运行记录（每次尝试的原因、日志尾部、本次运行完整控制台输出），按保留天数清理（默认 7 天，上限 180 天，每天自动清理）。
...
3. 校验并启动脚本主程序（工作目录 = 脚本根目录，参数按引号规则拆分），**不弹出新窗口**，控制台输出重定向捕获，运行结束后随历史按次完整保存（`history/YYYY-MM-DD/HH-mm-ss.console.log`）；若检测到脚本已在运行则直接监控其日志，不重复启动。
4. 持续监控日志（按用户填写的日志路径格式严格匹配，忽略运行前已有内容）：
...
5. 每次尝试结束清理进程树（`taskkill /T /F`，无论成败强制结束本次启动的脚本进程），按设置强制关闭游戏（须已勾选启动游戏）。
6. 未达最大尝试次数 → 自动重试；队列执行完一项进入下一项。
7. 队列全部跑完 → 执行完成操作。
...
| `history/YYYY-MM-DD/HH-mm-ss.console.log` | 同一次运行的完整控制台输出（stdout/stderr 捕获，与 `.json` 同名配对，20MB 截断） |
...
> 说明：脚本日志（history `.log`）与控制台输出（history `.console.log`）互不包含；历史/管理器日志按保留天数每天自动清理；`outputs/` 与旧聚合控制台日志（`logs/YYYY-MM-DD.log`）为历史遗留，新版本不再写入。

## 参与开发

- 协作方式以 v1.0.0 为界：之前直接提交并推送 `main`（无分支保护，版本发布为 Pre-release）；正式版 v1.0.0 起仅通过 Pull Request 合入。规范见 [CONTRIBUTING.md](CONTRIBUTING.md)；
