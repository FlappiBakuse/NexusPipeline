...
- **调度队列**：多个脚本按顺序链式执行；定时（按星期/时间）或启动时自动触发；全部跑完后可执行：退出软件 / 休眠 / 重启 / 关机（执行前 60 秒倒计时可取消）。任务失败也会照常执行完成操作（仅手动取消可跳过）。
...
**运行期间脚本自己生成的文件会被清理吗？**
会。多账号配置交换机制会在运行结束后还原配置目录现场——脚本写入配置目录内的文件会被删除（运行日志由 NexusPipeline 历史记录保全）。脚本数据请放在配置目录之外。
