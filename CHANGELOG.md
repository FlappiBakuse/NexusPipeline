...
- 自动签到占位卡片替换为「通用选项」，集中管理启用状态与运行天数。
...
- 展开卡片内提供编辑配置（1/1）、自动签到选项占位卡片（即将开发）、通知推送选项与高级选项（1/2）二级页；二级页提供返回上级。
...
- 自动签到入口改为绑定级占位卡片；用户管理界面移除用户级自动签到开关与空状态重复的「添加用户」按钮。
...
- 用户卡片提供绑定脚本数量、自动签到占位、下一轮调度倒计时、用户管理和删除入口；删除用户要求填写完整用户名确认。
- 在统一用户管理弹窗中配置脚本绑定、参与运行开关、前置/后置脚本、自动签到占位、用户通知开关和 SMTP 独立收件人。
