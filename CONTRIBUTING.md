...
2. 本地验证：`build.cmd` → 全量 e2e（`npx playwright test`，51 用例）全绿；CI 跑核心回归集（`$env:NEXUS_CI = "1"; npx playwright test`，50 用例）；专项测试 `node uitest\judge-scenarios.mjs`（116 断言）与 `node uitest\chaos-queue.mjs`（171 断言，需管理员 shell）；
